...
def user_dashboard(username):
    ...
    pages = ["🏠 Home", "📤 Upload Data", "📈 Blockchain Analytics", "📊 Visualization", "💱 Peer-to-Peer Transaction", "🌐 3D Visualization of Blockchain",   "🔗 Explore through API" , "🚪 Logout"]
    choice = st.sidebar.radio("Navigate to:", pages)

    if choice == "🏠 Home":
        home_page()  # Define the home page function
    elif choice == "📤 Upload Data":
        upload_transaction_data()  # Define the data upload function
    elif choice == "📈 Blockchain Analytics":
        if "uploaded_df" in st.session_state and st.session_state.uploaded_df is not None:
            blockchain_analytics(st.session_state.uploaded_df)  # Define blockchain analytics function
        else:
            st.warning("Please upload a transaction dataset first.")
    elif choice == "📊 Visualization":
        if "uploaded_df" in st.session_state and st.session_state.uploaded_df is not None:
            visualize_transaction_proportions_and_generate_report(st.session_state.uploaded_df)  # Define visualization function
        else:
            st.warning("Please upload a transaction dataset first.")
    elif choice == "💱 Peer-to-Peer Transaction":
        peer_to_peer_transaction()  # Call the peer-to-peer transaction function
    elif choice == "🌐 3D Visualization of Blockchain":
        if "uploaded_df" in st.session_state and st.session_state.uploaded_df is not None:
            visualize_blockchain_network(st.session_state.uploaded_df)  # Call the 3D visualization function
        else:
            st.warning("Please upload a transaction dataset first.")
        ...
        logout_user()  # Define logout functionality
...
def main():
    ...
    background_url = "https://technology.online.city.ac.uk/media/mageplaza/blog/post/b/l/blockchain_technology-2.jpg"
    st.markdown(
        f"""
        <style>
        .stApp {{
